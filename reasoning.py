...
#             mapping[var] = random_letter(variables)
...
for i in range(len(li)):
    res.append(Eliminate_implication(li[i]))
    res.append(De_morgans_law(li[i]))
    res.append(Remove_double_negation(li[i]))
    res.append(Standardize_variables(li[i]))
    res.append(prenix_phrase2(li[i]))
    # res.append(skolemization(li[i]))
    # res.append(distributiveLaws(li[i]))

print(eliminate_universal_quantifiers("(∀x P(x)) ∨ (∃y Q(y))"))
